...
const BudgetExecutionDashboard = ({ contractData, spendingMetrics, ministryData, filters, onFilterChange }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const competitiveRate = calculateCompetitiveBiddingRate(filteredContractData);
      const specializedFields = identifySpecializedFields(filteredContractData);
      const contractMethodChanges = calculateContractMethodChanges(filteredContractData);
      
      const insightText = getBudgetInsight({
        competitiveRate,
        specializedFields,
        contractMethodChanges
      }, filters);
