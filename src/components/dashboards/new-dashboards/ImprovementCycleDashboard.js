...
const ImprovementCycleDashboard = ({ reviewMetrics, ministryData, filters, onFilterChange }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const insightText = getImprovementInsight({
        improvementChange: calculateYearOverYearChange(filteredMetrics),
        longTermProjects: longTerm,
        reviewCaseCount: filteredMetrics.length,
      }, filters);
